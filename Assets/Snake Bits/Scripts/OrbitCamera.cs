...
public class OrbitCamera : MonoBehaviour {
	...
	void Update () {
		...
		RaycastHit hit;
		if ((Physics.Raycast (cam.transform.position, cam.transform.forward, out hit, Mathf.Infinity, 1 << 8) || Physics.Raycast (cam.transform.position, cam.transform.forward, out hit, Mathf.Infinity, 1 << 12)) && Input.GetMouseButtonDown (0)) {
			point = hit.point;
